...
This document enforces color fidelity and programmatic extraction of hex values. All hex values must derive from the provided `brand-guideline.png` and no other external references.
...
- **SBS Neutral Stone:** `#E7E3DC`
  - *Usage:* Card backgrounds (`.shop-card`, `.ys-community-card`). Creates soft separation from White Sand without needing drop shadows.
...
- **SBS Warm Mocha:** `#3F3229`
  - *Usage:* Heavy branding blocks, footer wrappers, or alternate ink tones for severe contrast.
- **SBS Black Slate:** `#2D2B27`
  - *Usage:* The primary text color (`--text-primary`). Primary solid button backgrounds (`.btn-solid`), extremely heavy section backgrounds (like The Story hero).
...
:root {
  ...
  --btn-bg-primary: var(--color-slate);
}
```
